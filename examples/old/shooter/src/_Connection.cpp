...
void Connection::set_socket(TCPsocket socket)
{
	if (_socket != nullptr)
	{
		SDLNet_FreeSocketSet(socket_set);
...
TCPsocket Connection::get_socket()
{
	return _socket;
}

bool Connection::send(Engine::NetworkPackage& package)
{
	return send(package.get_type(), package.serialize());
}

bool Connection::send(uint32_t data_type, Engine::span<uint8_t> data)
{
...
bool Connection::recv(uint32_t& data_type, Engine::span<uint8_t> data)
{
