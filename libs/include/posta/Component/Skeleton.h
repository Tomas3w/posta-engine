	...
	class Skeleton
	{
		public:
			...
			struct AnimationPlayer
			{
				...
				//std::vector<posta::anim_mat4> cache;
			};
			...
			posta::span<posta::anim_mat4> get_animation_matrices(AnimationPlayer& player, bool advance_time = true);
			/// Get animation matrices for multiple animations
			/** It does not work with bone weights individually */
			posta::span<posta::anim_mat4> get_blend_animation_matrices(AnimationPlayer& player, bool advance_time = true);
			...
			std::vector<posta::component::Transform> bone_transforms; // local to object, not to parent bone
		private:
			...
			struct Animation
			{
				...
				std::vector<std::vector<posta::component::Transform>> frames; // each frame contains all the bone transforms
			
				/// Returns frames of the animation but with the transforms being local to the parent bone
				std::vector<std::vector<posta::component::Transform>>& get_local_frames(std::vector<Bone>& root_bones);
			private:
				std::vector<std::vector<posta::component::Transform>> local_frames; // each frame contains all the local to parent bone transforms

			};
			std::unordered_map<std::string, std::unique_ptr<Animation>> animations;
			std::vector<posta::anim_mat4> models;
			std::vector<posta::anim_mat4> rest_matrices;
			std::vector<posta::component::Transform> rest_transforms;
	};
}
